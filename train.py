...
def f_measure(pred, target, beta_squared=0.3):
    """Calculate F-measure with given beta_squared value."""
    precision, recall = precision_and_recall(pred, target)

    f_measure_value = (1 + beta_squared) * precision * recall / (beta_squared * precision + recall + 1e-8)
    return f_measure_value


def e_measure(pred, target):
    """Calculate E-measure."""
    return np.mean(1 - np.abs(pred - target))
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='efgtnet', help='model name [default: ptsod]')
    parser.add_argument('--data_root', type=str, default='/home/liuk/pcsod_improve/data/', help='root path of data')
    parser.add_argument('--batch_size', type=int, default=16, help='Batch Size during training [default: 64]')
    parser.add_argument('--test_batch_size', type=int, default=1, help='Batch Size during evaluation [default: 32]')
    parser.add_argument('--epoch', default=1000, type=int, help='Epoch to run [default: 3000]')
    ...
    parser.add_argument('--gpu', type=str, default='0,1,2,3', help='GPU to use [default: GPU 0]')
    parser.add_argument('--optimizer', type=str, default='Adam', help='Adam or SGD [default: Adam]')
    parser.add_argument('--log_dir', type=str, default='efgtnet', help='Log path [default: None]')
    parser.add_argument('--decay_rate', type=float, default=1e-4, help='weight decay [default: 1e-4]')
    parser.add_argument('--npoint', type=int, default=4096, help='Point Number [default: 4096]')
    parser.add_argument('--step_size', type=int, default=200, help='Decay step for lr decay [default: every 10 epochs]')
    parser.add_argument('--lr_decay', type=float, default=0.7, help='Decay rate for lr decay [default: 0.7]')
    parser.add_argument('--eval', action='store_true', default=False, help='whether to eval[default: False]')
    parser.add_argument('--resume', action='store_true', default=True, help='Resume from checkpoint if set')
...
def main(args):
    ...
    trainDataLoader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=BATCH_SIZE, shuffle=True, num_workers=18,
    ...
    if args.eval:
        ...
        testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=TEST_BATCH_SIZE, shuffle=False,
                                                     num_workers=18,
                                                     pin_memory=True, drop_last=True)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        # for i, (points, target) in enumerate(trainDataLoader):
            ...
            iou_sum += jaccard_score((seg_o > 0.5).cpu().detach().numpy(), target.cpu().detach().numpy(),
                                     average='macro')
            fmeasure_sum += f_measure((seg_o > 0.5).cpu().detach().numpy(), target.cpu().detach().numpy(),
                                      beta_squared=0.3)
            emeasure_sum += e_measure(seg_o.cpu().detach().numpy(), target.cpu().detach().numpy())

            loss_sum += loss

        mae /= num_batches
        iou_avg = iou_sum / num_batches
        fmeasure_avg = fmeasure_sum / num_batches
        emeasure_avg = emeasure_sum / num_batches

        log_string('Training mean loss: %f' % (loss_sum / num_batches))
        log_string('Training MAE accuracy: %f' % mae)
        log_string('Training IoU accuracy: %f' % iou_avg)
        log_string('Training F-measure accuracy: %f' % fmeasure_avg)
        log_string('Training E-measure accuracy: %f' % emeasure_avg)
        ...
        if args.eval:
            with torch.no_grad():
                test_num_batches = len(testDataLoader)
                mae = 0
                iou_sum = 0
                fmeasure_sum = 0
                emeasure_sum = 0
                ...
                # for i, (points, target, scene_name) in tqdm(enumerate(testDataLoader), total=len(testDataLoader),  smoothing=0.9):
                for i, (points, target, scene_name) in enumerate(testDataLoader):
                    ...
                    final_pred /= NUM_VOTE
                    print("\n")
                    mae += Eval_mae(final_pred, target)
                    iou_sum += Eval_IoU(final_pred, target)
                    fmeasure_sum += Eval_fmeasure(final_pred, target)
                    emeasure_sum += Eval_Emeasure(final_pred, target)
                    print("mae:{}, iou_sum:{}, fmeasure_sum:{}, emeasure_sum:{}".format(mae, iou_sum, fmeasure_sum,
                                                                                        emeasure_sum))
                mae /= test_num_batches
                iou_avg = iou_sum / test_num_batches
                fmeasure_avg = fmeasure_sum / test_num_batches
                emeasure_avg = emeasure_sum / test_num_batches

                log_string('Eval MAE accuracy: %f' % mae)
                log_string('Eval IoU accuracy: %f' % iou_avg)
                log_string('Eval F-measure accuracy: %f' % fmeasure_avg)
                log_string('Eval E-measure accuracy: %f' % emeasure_avg)
